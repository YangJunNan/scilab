...
@echo on
call "%INSTALL_DIR%\bin\Scilex.exe" -scihome "%SCIHOME%" -quit -e "test_run('%TEST%',[],[],'%LOG_PATH%\%TEST%.xml'); [__msg__, __err__] = lasterror(), copyfile(TMPDIR+'/test_run_result/', '%LOG_PATH%\'); exit(__err__)"
if errorlevel 1 (
