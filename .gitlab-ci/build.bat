...
@REM tail -n 1 %LOG_PATH%\log_svn.txt
@REM REM revert local modification
@REM svn revert -R scilab >> %LOG_PATH%\log_svn.txt
...
bin\WScilex-cli.exe -nb -f "tools\innosetup\Create_ISS.sce" > ..\%LOG_PATH%\log_iss.txt
