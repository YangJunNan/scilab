...
if errorlevel 1 tail --lines=20 ..\log_svn.txt 1>&2 & exit 1
...
if errorlevel 1 tail --lines=20 ..\log_build.txt 1>&2 & exit 1
devenv Scilab.sln /build "Release|x64" /project buildhelp >..\log_buildhelp.txt |cmd /c ""
if errorlevel 1 tail --lines=20 ..\log_buildhelp.txt 1>&2 & exit 1
devenv Scilab.sln /build "Release|x64" /project buildjavadoc >..\log_buildjavadoc.txt |cmd /c ""
if errorlevel 1 tail --lines=20 ..\log_buildjavadoc.txt 1>&2 & exit 1

REM Delete previous builds
del "%CI_PROJECT_DIR%\scilab*.exe" >nul 2>&1
