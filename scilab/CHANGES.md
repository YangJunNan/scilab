...
This file details the changes between Scilab 2023.1.0 (this version), and the previous stable 6.1.1 release (including information from 2023.0.0).
...
Please report anything we could have missed, on the [mailing lists][3] or the [bug tracker][4].
...
* `edit_curv()` is upgraded. Newly implemented features:
  - Undo / Redo actions.
  - Deletion of points.
  - Editing a curve of a given (customized) handle.
  - Editing in given axes, persistent when leaving.
* Use `curl` as default on `atomsDownload()` for all OS to handle local file repositories.
* `cat()` accepts arrays of different but compatible types according to [,] and [;] rules.
* `tbx_make(".", "help", lang)` builds the documentation restricted to a given language.
* The MPI module documentation has been improved.

### Scilab 2023.0.0:

In summary, the main new features and additions are:
* `auto_stretch` property has been added for axes handles.
...
* Polynomial exponent display can either use ASCII or Unicode and is configurable through preferences (See issue [#16871](https://gitlab.com/scilab/scilab/-/issues/16871)).
* `toJSON(var, filename, indent)` implemented, after `toJSON(var, filename)`.
* `fromJSON` and `toJSON` processing of `tab` (`ascii(9)`) is changed.
* `for c = h,.., end` now accepts a hypermatrix `h` and iterates over its columns `matrix(h, size(h,1),-1)`.
...
* macOS
   - Scilab includes an embedded JRE and won't require a system-installed JDK at run-time (See issue  [#13087](https://gitlab.com/scilab/scilab/-/issues/13087)).
...
* [#8812](https://gitlab.com/scilab/scilab/-/issues/8812): Multi-selection in a listbox did not work when `max - min` was greater than 1.
...
* [#14358](https://gitlab.com/scilab/scilab/-/issues/14358): Black Hole demonstration produced a Java error when clicking on "Clear" button.
* [#15075](https://gitlab.com/scilab/scilab/-/issues/15075): `read()` was slow when reading an unknown number of lines (dims == -1).
* [#15709](https://gitlab.com/scilab/scilab/-/issues/15709): `languages` option has been added to `tbx_make(".","help")` to target building to some chosen languages.
* [#16136](https://gitlab.com/scilab/scilab/-/issues/16136): `sound()` is a useless wrapper of `playsnd()`. It is now obsolete before forthcoming removal.
* [#16442](https://gitlab.com/scilab/scilab/-/issues/16442): `splitURL()` has been moved to the webtools module and renamed to `url_split()`.
...
* [#16810](https://gitlab.com/scilab/scilab/-/issues/16810): `edit_curv()` has been fixed and upgraded.
* [#16838](https://gitlab.com/scilab/scilab/-/issues/16838): Xcos windows were not restored properly from `windowsConfiguration.xml`.
...
* [#16877](https://gitlab.com/scilab/scilab/-/issues/16877): `cellstr()` & `iscellstr()` now belongs to the `m2sci` module, where their code, pages and tests should be.
...
* [#16960](https://gitlab.com/scilab/scilab/-/issues/16960): `write()` now prints/writes an empty line in console/file when the input is an empty string.
* [#16962](https://gitlab.com/scilab/scilab/-/issues/16962): `getURL()` is now obsolete.
* [#16966](https://gitlab.com/scilab/scilab/-/issues/16966): The warning message for the removal of `xset()/xget()` did not tell which function displayed the message.
* [#16968](https://gitlab.com/scilab/scilab/-/issues/16968): `cov()` generated a warning (xerbla) or made Scilab crash (MKL generation issue).
* [#16969](https://gitlab.com/scilab/scilab/-/issues/16969): Scilab desktop default position was out of screen in WSL.
* [#16970](https://gitlab.com/scilab/scilab/-/issues/16970): Tests on `types::` in Webtools gateways and `http_upload()` example have been fixed.
* [#16980](https://gitlab.com/scilab/scilab/-/issues/16980): `fpf=' '` did not turn off labels in `contour()`.
...
* [#16986](https://gitlab.com/scilab/scilab/-/issues/16986): MPI module is now activated during build, build doc and packaging process.
* [#16987](https://gitlab.com/scilab/scilab/-/issues/16987): "Move object" icon was missing in the figures menubar under Linux/MacOS.
* [#16988](https://gitlab.com/scilab/scilab/-/issues/16988): Scilab did not start in case of wrong encoding of `SCI/etc/modules.xml`.
* [#16989](https://gitlab.com/scilab/scilab/-/issues/16989): `atomsSystemUpdate()` could no more be run in NWNI mode.
...
* [#16995](https://gitlab.com/scilab/scilab/-/issues/16995): `hist3d()` plotting feature no more worked in the variable browser and variable editor.
* [#17001](https://gitlab.com/scilab/scilab/-/issues/17001): AppStream metadata have been improved.
* [#17002](https://gitlab.com/scilab/scilab/-/issues/17002): `slint()` default configuration file is now copied into SCIHOME and used instead of the original one.
* [#17008](https://gitlab.com/scilab/scilab/-/issues/17008): The C code displayed in the `API Scilab/API double functions` documentation page has been fixed.
* [#17010](https://gitlab.com/scilab/scilab/-/issues/17010): `slint()` overwrote the default configuration file.
* [#17011](https://gitlab.com/scilab/scilab/-/issues/17011): `Matplot()` crashed when `rect` input argument was missing.
...
* [#17018](https://gitlab.com/scilab/scilab/-/issues/17018): The documentation for `x=input(…)` had an error (x type), and was poorly documented (description, examples).
* [#17022](https://gitlab.com/scilab/scilab/-/issues/17022): `kroneck()` returned a wrong error message.
* [#17041](https://gitlab.com/scilab/scilab/-/issues/17041): A wrong error message was returned when calling `mopen()` in read mode for a new file.
...
* [#16179](https://gitlab.com/scilab/scilab/-/issues/16179): `pause` inhibited error display when executed in a try-catch block.
...
* [#16755](https://gitlab.com/scilab/scilab/-/issues/16755): `plot3d` polygon facets rendering could be wrong when exactly in the horizontal or vertical plane.
...
* [#16822](https://gitlab.com/scilab/scilab/-/issues/16822): `getrelativefilename()` returned an incorrect value in case of the same beginning of directory names.
