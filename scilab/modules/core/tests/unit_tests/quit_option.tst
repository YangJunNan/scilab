...
exitcode = unix(scilabBin + " -quit -e ""1+1""");
...
exitcode = unix("< " + filepath + " """ + scilabBin + """ -quit");
assert_checkequal(exitcode, 0);
// without -quit option the behavior is the same
exitcode = unix("< " + filepath + " """ + scilabBin + """");
assert_checkequal(exitcode, 0);

exitcode = unix(scilabBin + " -quit -e ""error(''exit code will be set'')""");
assert_checktrue(exitcode <> 0);
