...
void SUNDIALSManager::computeFunction(types::typed_list &in, functionKind what, double *pdblOut, double *pdblOutExtra)
{
    ...
    for (int i=0; i < out.size(); i++)
    {
        ...
        m_odeIsComplex |= pDblOut[i]->isComplex();

        if (pDblOut[i]->getSize() != m_iSizeOfInput[what])
        {
            sprintf(errorMsg, _("%s: Wrong size for output argument #%d: expecting %d.\n"), m_pCallFunctionName[what], i, m_iSizeOfInput[what]);
            throw ast::InternalError(errorMsg);
        }
    }

    if (pdblOut != NULL) // pdblOut == NULL when testing if ode is complex
    {
...
void SUNDIALSManager::callClosing(functionKind what, types::typed_list &in, std::vector<int> iRetCount, types::typed_list &out)
{
    ...
    catch(ast::InternalError& ie)
    {
        ...
        {
            sprintf(errorMsg, wide_string_to_UTF8(ie.GetErrorMessage().c_str()));
            throw ast::InternalError(errorMsg);
        }
    }
...
void SUNDIALSManager::parseFunction(types::InternalType *pIn, functionKind what)
{
    ...
    else if (pIn->isList())
    {
        ...
        else if (pIFirst != NULL && pIFirst->isString())
        {
            ...
            else
            {
                ...
                if (func == NULL)
                {
                    sprintf(errorMsg,_("%ls: unable to find entry point %ls.\n"), m_wstrCaller.c_str(), pwstr);
                    throw ast::InternalError(errorMsg);
                }
            ...
            if (pList->getSize() > 2)
            {
                sprintf(errorMsg,_("%ls: only one parameter is allowed.\n"), m_wstrCaller.c_str(), pwstr);
                throw ast::InternalError(errorMsg);
            }
            if (pList->get(1)->isDouble() == false || pList->get(1)->getAs<types::Double>()->isComplex())
            {
                sprintf(errorMsg,_("%ls: parameter must be a real matrix.\n"), m_wstrCaller.c_str(), pwstr);
                throw ast::InternalError(errorMsg);
            }
            // mandatory IncreaseRef !
        ...
        {
            sprintf(errorMsg,_("%ls: first element of parameter %s should be a function or a string.\n"), m_wstrCaller.c_str(), pstrArgName[what]);
            throw ast::InternalError(errorMsg);
        }
    }
    ...
    else if ((what == JACY || what == MASS) && (pIn->isDouble() || pIn->isSparse()))
    {
        ...
        {
            sprintf(errorMsg, _("%ls: parameter %s should be a Double or Sparse matrix of size %d.\n"), m_wstrCaller.c_str(), pstrArgName[what], m_iSizeOfInput[what]);
            throw ast::InternalError(errorMsg);
        }
        pIn->IncreaseRef();
    ...
    else if (what == JACYYP && pIn->isCell())
    {
        ...
        {
            pCell->DecreaseRef();
            sprintf(errorMsg, _("%ls: parameter %s should be a cell with two Double or Sparse matrices of size %d x %d.\n"), m_wstrCaller.c_str(), pstrArgName[what], m_iNbEq, m_iNbEq);
            throw ast::InternalError(errorMsg);
        }
        pCell->IncreaseRef();
        pI0->IncreaseRef();
    ...
    else
    {
        if (what == JACYYP)
        {
            sprintf(errorMsg, _("%ls: parameter %s should be a matrix, a cell, a string, a function or a list.\n"), m_wstrCaller.c_str(), pstrArgName[what]);
        }
        else
        {
            sprintf(errorMsg, _("%ls: parameter %s should be a matrix, a string, a function or a list.\n"), m_wstrCaller.c_str(), pstrArgName[what]);
        }
        throw ast::InternalError(errorMsg);
    }
...
std::wstring SUNDIALSManager::setLinearSolver(functionKind what, N_Vector NV_work, SUNMatrix & SUN_A, SUNLinearSolver & SUN_LS)
{
    ...
    else if (m_iNonZeros[what] >= 0)
    {
        ...
        if (m_pIPattern[what] != NULL && m_pIPattern[what]->isSparse())
        {
            ...
            m_spJacEngine = new spCompJacobian(m_wstrCaller.c_str());
            m_spJacEngine->setPattern(m_pIPattern[what]);
            // do coloring of the graph
            if (m_spJacEngine->init() == false)
            {
                sprintf(errorMsg, _("%ls: m_spJacEngine->init() failed\n"), m_wstrCaller.c_str());
                throw ast::InternalError(errorMsg);
            }
    ...
    else if (m_wstrLinSolver==L"DENSE")
    {
