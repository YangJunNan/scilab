...
void copyComplexVectorToRealImg(double *pdblRealState, types::Double *pDbl, int iPos, int iSize, bool bComplex)
{
    int iTwo = 2;
    int iOne = 1;
...
void copyMatrixToSUNMatrix(types::InternalType *pI, SUNMatrix SUNMat_J, int iDim, bool bComplex)
{
    ...
    else if (pI->isSparse() && SUNMatGetID(SUNMat_J) == SUNMATRIX_SPARSE && SM_SPARSETYPE_S(SUNMat_J) == CSR_MAT)
    {
        ...
        if (bComplex)
        {
            long long int *piColIndexSUNMAT = SM_INDEXVALS_S(SUNMat_J); // column indexes of values
            long long int *piRowStartIndexSUNMAT = SM_INDEXPTRS_S(SUNMat_J); // index of first value of each row in values vector
