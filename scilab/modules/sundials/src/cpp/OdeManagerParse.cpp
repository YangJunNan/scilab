...
void OdeManager::parseMatrices(types::typed_list &in)
{
    ...
    if (in[1]->isDouble() && !in[1]->getAs<types::Double>()->isComplex())
    {
        ...
        if (m_odeIsExtension)
        {
            ...
            {
                sprintf(errorMsg, _("%ls: Wrong size for input argument #%d: expecting %d.\n"), m_wstrCaller.c_str(), 1, 1);
                throw ast::InternalError(errorMsg);
            }
            double dblTFinal = prevManager->m_dblVecTOut.back();
            if ( ((pDblTSpan->get(0)-dblTFinal)*(dblTFinal-prevManager->m_dblT0)) < 0)
            {
                sprintf(errorMsg, _("%ls: parameter #%d must be greater than %g.\n"), m_wstrCaller.c_str(), 2, dblTFinal);
                throw ast::InternalError(errorMsg);
            }
        ...
        if (m_pDblTSpan->getSize() > 2)
        {
            for (int i=2; i<m_pDblTSpan->getSize(); i++)
            {
                ...
                {
                    sprintf(errorMsg, _("%ls: values in parameter #%d should be monotone.\n"), m_wstrCaller.c_str(), 2);
                    throw ast::InternalError(errorMsg);
                }
            }
        }
    ...
    {
        sprintf(errorMsg, _("%ls: Wrong type for input argument #%d: A real matrix expected.\n"), m_wstrCaller.c_str(), 2);
        throw ast::InternalError(errorMsg);
    }
    ...
    if (isDAE())
    {
        ...
        {
            sprintf(errorMsg, _("%ls: y0 and yp0 must have the same size.\n"), m_wstrCaller.c_str());
            throw ast::InternalError(errorMsg);
        }
    }
...
void OdeManager::parseOptions(types::optional_list &opt)
{
    ...
    {
        getIntVectorInPlist(m_wstrCaller.c_str(), opt, L"jacBand", m_iVecBand[whatJAC],
        ...
        else if (m_functionAPI[whatJAC] == SUNDIALS_DLL)
        {
            ...
            getIntInPlist(m_wstrCaller.c_str(), opt, L"jacNonZeros", &m_iNonZeros[whatJAC],
                m_odeIsExtension ? prevManager->m_iNonZeros[whatJAC] : -1, {0, m_iNbEq*m_iNbEq});
            if (m_iNonZeros[whatJAC] >= 0)
            {
                getStringInPlist(m_wstrCaller.c_str(), opt, L"jacFormat", m_wstrSparseFormat[whatJAC],
    ...
    getIntInPlist(m_wstrCaller.c_str(), opt, L"nonLinSolMaxIters", &m_iNonLinSolMaxIters,
        m_odeIsExtension ? prevManager->m_iNonLinSolMaxIters : 0, {0, INT_MAX});
    getIntInPlist(m_wstrCaller.c_str(), opt, L"nonLinSolAccel", &m_iNonLinSolAccel,
        m_odeIsExtension ? prevManager->m_iNonLinSolAccel : 0, {0, INT_MAX});

    // parse linear solver
    getStringInPlist(m_wstrCaller.c_str(), opt, L"linearSolver", m_wstrLinSolver, 
    ...
    {
        getIntInPlist(m_wstrCaller.c_str(), opt, L"linSolMaxIters", &m_iLinSolMaxIters,
            m_odeIsExtension ? prevManager->m_iLinSolMaxIters : 0, {0, INT_MAX});
        if (hasBandPrec())
        {
            getIntVectorInPlist(m_wstrCaller.c_str(), opt, L"precBand", m_iPrecBand,
                m_odeIsExtension ? prevManager->m_iPrecBand : emptyVect, {0, m_iNbEq-1}, {2});                
        }
        getStringInPlist(m_wstrCaller.c_str(), opt, L"precType", m_wstrPrecondType,
    ...
    if (m_odeIsExtension)
    {
        ...
        if (opt.find(L"y0") != opt.end())
        {
            if (opt[L"y0"]->isDouble())
            {
                ...
                if (bSameDims == false)
                {
                    sprintf(errorMsg, _("%ls: Wrong size for option \"y0\": expecting %d x %d.\n"), m_wstrCaller.c_str(), m_pDblY0->getRows(), m_pDblY0->getCols());
                    throw ast::InternalError(errorMsg);
                }
                if (prevManager->m_odeIsComplex == false && pDbl->isComplex())
                {
                    sprintf(errorMsg, _("%ls: Wrong type (complex) for option \"y0\": extended ode is real.\n"), m_wstrCaller.c_str());
                    throw ast::InternalError(errorMsg);
                }
                m_pDblY0->killMe();
                m_pDblY0 = pDbl->clone();
            ...
            {
                sprintf(errorMsg, _("%ls: Wrong type for option \"y0\": a double vector is expected.\n"), m_wstrCaller.c_str());
                throw ast::InternalError(errorMsg);
            }
        }
        ...
        if (isDAE() && opt.find(L"yp0") != opt.end())
        {
            if (opt[L"yp0"]->isDouble())
            {
                ...
                if (bSameDims == false)
                {
                    sprintf(errorMsg, _("%ls: Wrong size for option \"y0\": expecting %d x %d.\n"), m_wstrCaller.c_str(), m_pDblYp0->getRows(), m_pDblYp0->getCols());
                    throw ast::InternalError(errorMsg);
                }
                if (prevManager->m_odeIsComplex == false && pDbl->isComplex())
                {
            ...
            {
                sprintf(errorMsg, _("%ls: Wrong type for option \"yp0\": a double vector is expected.\n"), m_wstrCaller.c_str());
                throw ast::InternalError(errorMsg);
            }
        }
    ...
    else
    {
        ...
        if (m_pDblTSpan->getSize() > 1 )
        {
            ...
            {
                getDoubleInPlist(m_wstrCaller.c_str(), opt, L"t0", &m_dblOptT0,
        ...
        {
            getDoubleInPlist(m_wstrCaller.c_str(), opt, L"t0", &m_dblOptT0, m_dblOptT0, {});
        }
    ...
    {
        sprintf(errorMsg, _("%ls: Wrong type (complex) for option \"f\" or \"r\": extended ode is real.\n"), m_wstrCaller.c_str());
        throw ast::InternalError(errorMsg);
    }
