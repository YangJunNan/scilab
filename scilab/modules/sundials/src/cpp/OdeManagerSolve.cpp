...
void OdeManager::solve()
{
    ...
    for (; m_dblT0 != dblFinalTime; iStep++)
    {
        ...
        if (iFlag == ODE_SUCCESS || iFlag == ODE_TSTOP_RETURN || iFlag == ODE_ROOT_RETURN)
        {
            if (dblTime == dblPrevTime)
            {
                ...
                errHandler(fromODEReturn[ODE_WARNING], NULL, NULL, errorMsg);
                break;
            }
            if (m_iRetCount > 0)
            {
            ...
            if (bTerminalEvent == true
                ...
                || (m_bHas[INTCB] && SUNDIALSIntCb(dblTime, iFlag == ODE_ROOT_RETURN ? 1 : 0, m_N_VectorY, m_N_VectorYp, (void *)this))
    ...
    if (m_bHas[INTCB])
    {
        SUNDIALSIntCb(dblTime, 2, m_N_VectorY, m_N_VectorYp, (void *)this);
    }
...
void OdeManager::errHandler(int error_code, const char *module, const char *function, char *msg)
{
    ...
    if (msg == NULL)
    {
        ...
        sprintf(errorMsg, "%ls: at t=%g, %s\n",getCallerName().c_str(), dblTime, getReturnFlagName(error_code));
    }
    else
    {
        sprintf(errorMsg, "%ls: %s", getCallerName().c_str(), msg);        
    }
