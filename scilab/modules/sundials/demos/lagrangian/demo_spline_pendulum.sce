...
function initialize()
    ...
    gcf().info_message="Drag and drop curve and pendulum points then click Start button"
    gcf().event_handler="handler"
    gcf().event_handler_enable="on"
    gcf().user_data.c=0;
    gcf().user_data.sp=sp;
    gcf().user_data.hdl=[hn,hs,hp,hc,hpt];
    gca().data_bounds=[-2 2 -2 2]
    isoview on
    uicontrol("string","Start","tag","button","callback","start()");
end
