...
tspan = [0 100];
[t,y,yp]=ida(list(res,K,M),tspan,y0,yp0);
assert_checktrue(max(abs(yend-y(:,$)))<1e-6)
assert_checktrue(max(abs(res(t,y,yp,K,M))) < 1e-4)

[t,yj,ypj]=ida(list(res,K,M),tspan,y0,yp0,jacobian={-K,M});
assert_checktrue(max(abs(yend-yj(:,$)))<1e-6)

[t,yjf,ypjf]=ida(list(res,K,M),tspan,y0,yp0,jacobian=list(jac,K,M));
assert_checktrue(max(abs(yend-yjf(:,$)))<1e-6)

[t,yb,ypb,info]=ida(list(res,K,M),tspan,y0,yp0,jacBand=[2,2]);
assert_checktrue(max(abs(yend-yb(:,$)))<1e-6)

[t,ys,yps]=ida(list(res,K,M),tspan,y0,yp0,jacobian={-sparse(K),sparse(M)});
assert_checktrue(max(abs(yend-ys(:,$)))<1e-6)

[t,ysf,ypsf]=ida(list(res,K,M),tspan,y0,yp0,jacobian=list(jacsp,K,M));
