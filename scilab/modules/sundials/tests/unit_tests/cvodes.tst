...
[t,y,s1] = cvode(vdp, 0:0.1:10, y0, sensPar=mu,rtol=1e-10);
[t,y,s2] = cvode(vdp, 0:0.1:10, y0, sensPar=mu,rtol=1e-10,sensErrCon=%t);

assert_checktrue(max(abs(squeeze(ys(:,2,:))-s1))<2e-4)
assert_checktrue(max(abs(squeeze(ys(:,2,:))-s2))<5e-5)
...
tspan=0:800;
y0=[1-1e-6;1e-6;0];
[t,y,s]=cvode(sir,tspan,y0,sensPar=[bet;gam],method="BDF");
assert_checkequal(size(s)(1:2),[3 2])

[t,y,s]=cvode(sir,tspan,y0,sensPar=[gam;bet],method="BDF");
