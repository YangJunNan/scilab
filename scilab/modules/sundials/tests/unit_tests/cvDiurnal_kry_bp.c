...
static void PrintOutput(void *cvode_mem, N_Vector u, realtype t)
{
  ...
  printf("t = %.2e   no. steps = %ld   order = %d   stepsize = %.2e\n",
         t, nst, qu, hu);
  printf("c1 (bot.left/middle/top rt.) = %24.17e  %24.17e  %24.17e\n",
         IJKth(udata,1,0,0), IJKth(udata,1,mxh,myh), IJKth(udata,1,mx1,my1));
  printf("c2 (bot.left/middle/top rt.) = %24.17e  %24.17e  %24.17e\n\n",
         IJKth(udata,2,0,0), IJKth(udata,2,mxh,myh), IJKth(udata,2,mx1,my1));
#else
  printf("t = %.2e   no. steps = %ld   order = %d   stepsize = %.2e\n",
         t, nst, qu, hu);
...
int f(realtype t, N_Vector u, N_Vector udot, void *user_data)
{
  ...
  // cast double array coming from Scilab to UserData type
  // this should work provided no padding is added between struct members
  UserData data = (UserData)((double *)user_data);
