...
[t,y,yp,te,ye,ype] = ida(list(sir,bet,gam,N),[0 400],y0,yp0,events=list(sir_ev,bet,gam,N));
assert_checkalmostequal(te,114.56018)
assert_checkalmostequal(ype(2),0,0,1e-6)
...
[t,y,yp] = ida(list(res,A,B),[0 10],y0,zeros(2,1),calcIc="y0yp0");
assert_checkalmostequal(yp(:,1),yp0);
...
assert_checkalmostequal(yp(:,1),0.816496582)
...
tic;
for i=1:10
[t1,y1,yp1] = ida("SUN_chemres",tspan,y0,yp0);
end
et1=toc();
tic;
for i=1:10
[t2,y2,yp2] = ida(robertsidae,tspan,y0,yp0);
end
et2=toc();
tic;
for i=1:10
[t3,y3,yp3] = ida("SUN_chemres",tspan,y0,yp0,jacobian="SUN_chemjac");
end
et3=toc();

assert_checkequal(t1,t2)
assert_checkequal(y1,y2)
assert_checkequal(yp1,yp2)
...
//test dynamic event
[t3,y3,yp3,te,ye] = ida("SUN_chemres",tspan,y0,yp0,jacobian="SUN_chemjac",events="SUN_chemevent",nbEvents=1);
assert_checkequal(te,4.84950409816763735D-03)

//tes dynamic callback
[t1,y1,yp1] = ida("SUN_chemres",tspan,y0,yp0,callback="SUN_chemcb");
