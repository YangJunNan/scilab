...
assert_checkalmostequal(y(:,$),[-1.50611787646778961; 1.18737949707269606D-03]);
...
// EVENTS
[t,y,te,ye,ie] = cvode(list(vdp,mu), [0,10], y0, method="BDF",jacobian=list(vdpJac,mu),events=list(myevent,0,0));
assert_checkalmostequal(ye(1,:),[1.7,1.7 1.7])
assert_checkalmostequal(te,[ 1.21746186   6.668978609   7.860696911])
//
[t,y,te,ye,ie] = cvode(list(vdp,mu), [0,10], y0, method="BDF",jacobian=list(vdpJac,mu),events=list(myevent,0,-1));
assert_checkalmostequal(ye(1,:),[1.7, 1.7])
assert_checkalmostequal(te,[ 1.21746186    7.860696911])
//
[t,y,te,ye,ie] = cvode(list(vdp,mu), [0,10], y0, method="BDF",jacobian=list(vdpJac,mu),events=list(myevent,0,-1));
assert_checkalmostequal(ye(1,:),[1.7, 1.7])
assert_checkalmostequal(te,[ 1.21746186    7.860696911])
//
[t,y,te,ye,ie] = cvode(list(vdp,mu), [0,10], y0, method="BDF",jacobian=list(vdpJac,mu),events=list(myevent,1,1));
assert_checkequal(te, t($))
assert_checkalmostequal(ye(1,:),[1.7])
assert_checkalmostequal(te,[6.668978609])
//
[t,y,te,ye,ie] = cvode(list(vdp,mu), [0,10], y0, method="BDF",jacobian=list(vdpJac,mu),events=list(myevent,0,0));
...
assert_checkequal(te,sol.te)
assert_checkequal(ye,sol.ye)
assert_checkequal(ie,sol.ie)
...
assert_checkalmostequal(solc.y(:,$),complex( 1.00009192413402314692,-0.000078229167544612501))
...
te = 0;
while %t
    [t,y,tev,ye] = cvode(list(bounce,9.81), [te($) 100], yini, events=bounce_ev, refine=16);
    if tev==[]
        break;
    end
...
assert_checkalmostequal(te($), 4.0588273);
//drawnow
...
bet=0.2;
y0=[N-1;1;0];
[t,y,te,ye]=cvode(list(sir,bet,gam,N),[0 400],y0,events=sir_ev);
assert_checkalmostequal(te, 114.561646320925021314);
assert_checkalmostequal(ye,[7499999.999998786486685
...
// EVENTS
[t,y,te,ye] = cvode(list(vdp,1), [0 10], [2;1], events="SUN_dynevent",nbEvents=1,evTerm=1);
assert_checkalmostequal(y(1,$),1.7)

[t,y,te,ye] = cvode(list(vdp,1), [0 10], [2;1], events="SUN_dynevent",nbEvents=1,evDir=1)
assert_checkequal(size(ye),[2 1])

[t,y,te,ye] = cvode(list(vdp,1), [0 10], [2;1], events="SUN_dynevent",nbEvents=1,evDir=-1)
assert_checkequal(size(ye),[2 2])

// EVENTS WITH PARAMETER
[t,y,te,ye] = cvode(list(vdp,1), [0 10], [2;1], events=list("SUN_dyneventpar",1),nbEvents=1,evTerm=1);
assert_checkalmostequal(ye(1),1)

[t,y,te,ye] = cvode(list(vdp,1), [0 10], [2;1], events=list("SUN_dyneventpar",1.234),nbEvents=1,evTerm=1);
assert_checkalmostequal(ye(1),1.234)
