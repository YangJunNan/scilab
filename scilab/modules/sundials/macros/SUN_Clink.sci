...
function _lib = SUN_Clink(names,files_in,p1,p2,p3,p4,p5,p6,p7,p8,p9)
    ...
    CFLAGS = "";
    if getos() == "Windows"
        SUNDIALSpath = fullfile(SCI,'modules','sundials','includes');
        CFLAGS=" -I"+SUNDIALSpath;
    else
        // Source tree version
        if isdir(SCI+"/modules/sundials/src/")
            SUNDIALSpath = fullfile(SCI,'modules','sundials','src','patched_sundials','include');
            CFLAGS=" -I"+SUNDIALSpath;
        end
    end
